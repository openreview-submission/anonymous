...
from .utils import cos_approx, sin_approx, sigmoid_approx, log_kernel_autotune
...
@triton.autotune(
    ...
    key=["HEADDIM_V"],
    prune_configs_by={'early_config_prune': log_kernel_autotune},
)
@triton.jit
def mamba3_siso_bwd_kernel_dzdo(
    ...
    # Strides for Dz: (batch, seqlen, nheads, headdim_v) — output strides as scalars
    ...
    # Dimensions
    seqlen, headdim_v,
    # Output tensors (appended by jax_triton_kda.triton_call from out_shape)
    Dz, DO_scaled,
...
def mamba3_siso_bwd_kernel_dqkv(
    ...
    for chunk_idx_loop in range(num_chunks):
        ...
        # Sequence-length mask for scalar loads/stores
        offs_cs = chunk_start + tl.arange(0, CHUNK_SIZE)
        seq_mask = offs_cs < seqlen

        # ============================================================
        # Load Decay Values
        # We load these first to overlap computation with block pointer loads
        # ============================================================
        ...
        # ============================================================
        # Load Q, K, V, dO, SSM_States via block pointers
        # ============================================================
        do_block = tl.load(tl.make_block_ptr(dO + do_offset, shape=(seqlen, headdim_v), strides=(stride_do_seqlen, stride_do_vdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_V), order=(1, 0)), boundary_check=(0,))  # (CHUNK_SIZE, HEADDIM_V)
        v_block = tl.load(tl.make_block_ptr(V + v_offset, shape=(seqlen, headdim_v), strides=(stride_v_seqlen, stride_v_vdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_V), order=(1, 0)), boundary_check=(0,))  # (CHUNK_SIZE, HEADDIM_V)
        q_block = tl.load(tl.make_block_ptr(Q + q_offset, shape=(seqlen, headdim_qk), strides=(stride_q_seqlen, stride_q_qkdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_QK), order=(1, 0)), boundary_check=(0,))  # (CHUNK_SIZE, HEADDIM_QK)
        k_block = tl.load(tl.make_block_ptr(K + k_offset, shape=(seqlen, headdim_qk), strides=(stride_k_seqlen, stride_k_qkdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_QK), order=(1, 0)), boundary_check=(0,))  # (CHUNK_SIZE, HEADDIM_QK)
        ssm_states_block = tl.load(tl.make_block_ptr(SSM_States + ssm_states_offset, shape=(headdim_v, num_chunks * headdim_qk), strides=(stride_ssm_states_vdim, stride_ssm_states_qkdim), offsets=(0, chunk_idx * headdim_qk), block_shape=(HEADDIM_V, HEADDIM_QK), order=(1, 0)), boundary_check=(0,))  # (HEADDIM_V, HEADDIM_QK)
        ...
        tl.store(tl.make_block_ptr(dK + dk_offset, shape=(seqlen, headdim_qk), strides=(stride_dk_seqlen, stride_dk_qkdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_QK), order=(1, 0)), acc_dk.to(k_block.dtype), boundary_check=(0,))
        ...
        if RECOMPUTE_MASK:
            ...
            s_block = tl.where(
                tl.arange(0, CHUNK_SIZE)[None, :] > tl.arange(0, CHUNK_SIZE)[:, None],
                s_block,
                tl.zeros_like(s_block),
            )
        else:
            s_block *= causal_decay_mask

        acc_dq = tl.dot(tl.trans(s_block).to(k_block.dtype), k_block).to(tl.float32)  # (CHUNK_SIZE, HEADDIM_QK)

        # Inter-chunk: gradient through states from previous chunks
        acc_dq += tl.dot(do_block, ssm_states_block.to(do_block.dtype)).to(tl.float32) * exp_da_cs[:, None]

        tl.store(tl.make_block_ptr(dQ + dq_offset, shape=(seqlen, headdim_qk), strides=(stride_dq_seqlen, stride_dq_qkdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_QK), order=(1, 0)), acc_dq.to(q_block.dtype), boundary_check=(0,))
        ...
        acc_dv = tl.dot(p_t_block.to(do_block.dtype), do_block).to(tl.float32)  # (CHUNK_SIZE, HEADDIM_V)

        # Inter-chunk: gradient through states
        acc_dv += tl.dot(k_block, tl.trans(d_ssm_states_acc).to(k_block.dtype)).to(tl.float32) * exp_da_cs_rev[:, None]
        ...
        tl.store(tl.make_block_ptr(dV + dv_offset, shape=(seqlen, headdim_v), strides=(stride_dv_seqlen, stride_dv_vdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_V), order=(1, 0)), acc_dv.to(v_block.dtype), boundary_check=(0,))
        ...
        # dQK_dot = sum_v(dO * V) for each position (row-wise sum)
        dQK_dot_block = tl.sum(dO_reloaded * v_block_reloaded, axis=1).to(tl.float32)
        ...
        # Accumulate dD gradient (sum over all positions in chunk)
        if D is not None:
            dD_acc += tl.sum(dQK_dot_block).to(tl.float32)
        ...
        d_ssm_states_acc = (tl.math.exp2(da_cs_chunk_sum) * d_ssm_states_acc +
                       tl.dot(tl.trans(dO_reloaded).to(q_block.dtype), q_block).to(tl.float32))
...
def mamba3_siso_bwd_kernel_ddt_dtrap_dinput_states(
    ...
    stride_cu_seqlen,
    # Strides for dDT: (batch, nheads, seqlen) — output strides as scalars
