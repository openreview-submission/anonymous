...
from .utils import cos_approx, sin_approx, tanh_approx, silu, sigmoid_approx, log_kernel_autotune
...
def mamba3_siso_fwd_kernel(
    ...
    # Outputs (appended by jax_triton_kda.triton_call from out_shape)
    ...
    # Block pointers are created inline at each load/store site (Triton 3.4 compat)
    ...
    for chunk_idx in range(num_chunks):
        ...
        # Load Q and K blocks
        q_pre_block = tl.load(tl.make_block_ptr(q_ptr, shape=(seqlen, headdim_qk), strides=(stride_q_seqlen, stride_q_qkdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_QK), order=(1, 0)), boundary_check=(0,))
        k_pre_block = tl.load(tl.make_block_ptr(k_ptr, shape=(seqlen, headdim_qk), strides=(stride_k_seqlen, stride_k_qkdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_QK), order=(1, 0)), boundary_check=(0,))
        ...
        # Compute QK dot products for skip connection (row-wise sum of Q*K)
        store_qk_dot = tl.sum(q_pre_block * k_pre_block, axis=1).to(tl.float32)
        store_qk_dot *= gamma
        ...
        tl.store(tl.make_block_ptr(k_store_ptr, shape=(seqlen, headdim_qk), strides=(stride_k_store_seqlen, stride_k_store_qkdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_QK), order=(1, 0)), k_pre_block.to(K.dtype.element_ty), boundary_check=(0,))
        ...
        tl.store(tl.make_block_ptr(q_store_ptr, shape=(seqlen, headdim_qk), strides=(stride_q_store_seqlen, stride_q_store_qkdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_QK), order=(1, 0)), q_pre_block.to(Q.dtype.element_ty), boundary_check=(0,))
    ...
    for chunk_idx in range(num_chunks):
        ...
        q_block = tl.load(tl.make_block_ptr(q_store_ptr, shape=(seqlen, headdim_qk), strides=(stride_q_store_seqlen, stride_q_store_qkdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_QK), order=(1, 0)), boundary_check=(0,))
        k_block = tl.load(tl.make_block_ptr(k_store_ptr, shape=(seqlen, headdim_qk), strides=(stride_k_store_seqlen, stride_k_store_qkdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_QK), order=(1, 0)), boundary_check=(0,))
        v_block = tl.load(tl.make_block_ptr(v_ptr, shape=(seqlen, headdim_v), strides=(stride_v_seqlen, stride_v_vdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_V), order=(1, 0)), boundary_check=(0,))
        if HAS_Z:
            z_block = tl.load(tl.make_block_ptr(z_ptr, shape=(seqlen, headdim_v), strides=(stride_z_seqlen, stride_z_vdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_V), order=(1, 0)), boundary_check=(0,))
        ...
        # Store output
        tl.store(tl.make_block_ptr(o_ptr, shape=(seqlen, headdim_v), strides=(stride_o_seqlen, stride_o_vdim), offsets=(chunk_start, 0), block_shape=(CHUNK_SIZE, HEADDIM_V), order=(1, 0)), acc_o.to(V.dtype.element_ty), boundary_check=(0,))

        if STORE_SSM_STATES_ADT_OUTV:
            tl.store(tl.make_block_ptr(ssm_states_ptr, shape=(headdim_v, num_chunks * headdim_qk), strides=(stride_ssm_states_vdim, stride_ssm_states_qkdim), offsets=(0, chunk_idx * headdim_qk), block_shape=(HEADDIM_V, HEADDIM_QK), order=(1, 0)), acc_ssm_states.to(tl.bfloat16), boundary_check=(0,))
