...
# otypes=[np.int32] lets np.vectorize handle size-0 inputs without raising.
# Without it: ValueError "cannot call vectorize on size 0 inputs unless otypes is set",
# which fires when book is (0, 43) — happens if a rare file falls back to the
# zero-row placeholder. The downstream output shape is (k, price_levels) with k=0,
# which downstream code can absorb.
@partial(np.vectorize, signature="(c),(),()->(d)", otypes=[np.int32])
