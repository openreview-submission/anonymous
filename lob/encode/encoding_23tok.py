...
# event_dir	price	size	delta_t	time_s	time_ns	price_ref	size_ref	time_s_ref	time_ns_ref

# ============================================
# 23-TOKEN MODE: Fused event_dir encoding
# event_dir = (event_type - 1) * 2 + direction  (8 values: 0-7)
# Reverse: event_type = event_dir // 2 + 1, direction = event_dir % 2
# ============================================
@jax.jit
def encode_msg_23(
        msg: jax.Array,
    ...
    # Fuse event_type (msg[1]) and direction (msg[2]) into single token
    event_dir_raw = (msg[1] - 1) * 2 + msg[2]
    event_dir = encode(event_dir_raw, *encoding['event_dir'])
    ...
    # 23-token output: event_dir is 1 token (was 2: event_type + direction)
    out = [
        event_dir, price_sign, price, size_high_enc, size_low_enc, time_comb,
        price_ref_sign, price_ref, size_ref_high_enc, size_ref_low_enc, time_ref_comb]
    return jnp.hstack(out)


# Vectorized version
encode_msg = encode_msg_23
encode_msgs = jax.jit(jax.vmap(encode_msg_23, in_axes=(0, None)))
...
# ============================================
# 23-TOKEN MODE: Decode with event_dir split back to event_type + direction
# ============================================
@jax.jit
def decode_msg_23(msg_enc, encoding):
    # Split fused event_dir back into event_type and direction
    event_dir = decode(msg_enc[0], *encoding['event_dir'])
    event_type = event_dir // 2 + 1
    direction = event_dir % 2

    price_sign = decode(msg_enc[1], *encoding['sign'])
    price = decode(msg_enc[2], *encoding['price'])
    price = combine_field(price, 3, price_sign)

    # Base-100 decoding for size
    size_high = decode(msg_enc[3], *encoding['size_digit'])
    size_low = decode(msg_enc[4], *encoding['size_digit'])
    size = size_high * 100 + size_low

    # 23-token indices: time is at 5:14
    ...
    size_ref_high = decode(msg_enc[16], *encoding['size_digit'])
    size_ref_low = decode(msg_enc[17], *encoding['size_digit'])
    size_ref = size_ref_high * 100 + size_ref_low

    time_s_ref, time_ns_ref = decode_time(msg_enc[18:23], encoding)
...
# Vectorized version
decode_msg = decode_msg_23
decode_msgs = jax.jit(jax.vmap(decode_msg_23, in_axes=(0, None)), backend='cpu')
...
class Vocab:
    ...
    def __init__(self) -> None:
        ...
        # 23tok: fused event_dir = (event_type - 1) * 2 + direction (8 values)
        self._add_field('event_dir', range(8), None)

        # 24tok-style base-100 encoding for size (two tokens: high/low digits)
...
class Message_Tokenizer:
    ...
    # Token lengths: 23-token mode (fused event_dir, base-100 size encoding)
    # Layout: [event_dir:0, price_sign:1, price:2, size:3-4, dt_s:5, dt_ns:6-8,
    #          time_s:9-10, time_ns:11-13, price_ref_sign:14, price_ref:15,
    #          size_ref:16-17, time_ref:18-22]
    TOK_LENS = np.array((1, 2, 2, 1, 3, 2, 3, 2, 2, 2, 3))  # sum=23
    ...
    # 23tok: [event_dir:0, price_sign:1, price:2, size:3-4, dt_s:5, dt_ns:6-8, time_s:9-10, time_ns:11-13, ...]
    TIME_START_I = 9
    TIME_END_I = 13
