...
# ============================================
# 26-TOKEN MODE: Base-1000 price (2 tokens), base-100 size (2 tokens)
# ============================================
# Extends 24tok by splitting price from 1 token to 2 tokens base-1000:
#   - price:  1 token (range 0-999) -> 2 tokens base-1000 (range 0-999,999 = +/-$9,999.99)
#   - size:   unchanged at 2 tokens base-100 (range 0-9999, 0.01% saturated)
# price_ref gets the same treatment -> +2 tokens total (24 -> 26).
# Vocab identical to 24tok: 2112 (reuses price field with range(1000)).
# Price effectively lossless: +/-$9,999.99 from mid covers all orders across all tickers.
@jax.jit
def encode_msg_26(
        msg: jax.Array,
    ...
    # Base-1000 encoding for price: sign + 2 base-1000 digits (range 0-999,999)
    # split_field(x, 2, 3, True) -> [sign, x//1000, x%1000]
    price = split_field(msg[4], 2, 3, True)
    price_sign = encode(price[0], *encoding['sign'])
    price_high = encode(price[1], *encoding['price'])
    price_low = encode(price[2], *encoding['price'])

    # Base-100 encoding for size: 2 base-100 digits (range 0-9999, same as 24tok)
    size_high = msg[5] // 100
    size_low = msg[5] % 100
    ...
    # Base-1000 encoding for price_ref
    price_ref = split_field(msg[10], 2, 3, True)
    price_ref_sign = encode(price_ref[0], *encoding['sign'])
    price_ref_high = encode(price_ref[1], *encoding['price'])
    price_ref_low = encode(price_ref[2], *encoding['price'])
    ...
    # 26-token output: price 3 toks (sign+2), size 2 toks, price_ref 3, size_ref 2
    out = [
        event_type, direction,
        price_sign, price_high, price_low,
        size_high_enc, size_low_enc,
        time_comb,
        price_ref_sign, price_ref_high, price_ref_low,
        size_ref_high_enc, size_ref_low_enc,
        time_ref_comb]
    return jnp.hstack(out)


# Vectorized version (26-token)
encode_msg = encode_msg_26
encode_msgs = jax.jit(jax.vmap(encode_msg_26, in_axes=(0, None)))
...
# ============================================
# 26-TOKEN MODE: Base-1000 decoding for price, base-100 for size
# ============================================
@jax.jit
def decode_msg_26(msg_enc, encoding):
    event_type = decode(msg_enc[0], *encoding['event_type'])
    direction = decode(msg_enc[1], *encoding['direction'])

    # Base-1000 decoding for price: sign + 2 digits (each 0-999)
    price_sign = decode(msg_enc[2], *encoding['sign'])
    price_high = decode(msg_enc[3], *encoding['price'])
    price_low = decode(msg_enc[4], *encoding['price'])
    price = combine_field(jnp.stack([price_high, price_low]), 3, price_sign)

    # Base-100 decoding for size: 2 digits
    size_high = decode(msg_enc[5], *encoding['size_digit'])
    size_low = decode(msg_enc[6], *encoding['size_digit'])
    size = size_high * 100 + size_low

    # 26-token indices: time block is at 7:16 (9 tokens: dt_s + dt_ns*3 + ts*2 + tns*3)
    delta_t_s, delta_t_ns, time_s, time_ns = decode_time(msg_enc[7:16], encoding)

    # Base-1000 decoding for price_ref: sign + 2 digits
    price_ref_sign = decode(msg_enc[16], *encoding['sign'])
    price_ref_high = decode(msg_enc[17], *encoding['price'])
    price_ref_low = decode(msg_enc[18], *encoding['price'])
    price_ref = combine_field(jnp.stack([price_ref_high, price_ref_low]), 3, price_ref_sign)

    # Base-100 decoding for size_ref: 2 digits
    size_ref_high = decode(msg_enc[19], *encoding['size_digit'])
    size_ref_low = decode(msg_enc[20], *encoding['size_digit'])
    size_ref = size_ref_high * 100 + size_ref_low

    time_s_ref, time_ns_ref = decode_time(msg_enc[21:26], encoding)
...
# Vectorized version (26-token)
decode_msg = decode_msg_26
decode_msgs = jax.jit(jax.vmap(decode_msg_26, in_axes=(0, None)), backend='cpu')
...
class Vocab:
    ...
    def __init__(self) -> None:
        ...
        # 26tok: Base-100 size (same as 24tok), vocab_size=2112
        self._add_field('size_digit', range(100), [])

        # 26tok: Price uses range(1000) — same field as 24tok, but now 2 tokens per price
...
class Message_Tokenizer:
    ...
    # Token lengths: 26-token mode (base-1000 price, base-100 size)
    # Layout: [evt:0, dir:1, price:2-4, size:5-6, dt_s:7, dt_ns:8-10, time_s:11-12, time_ns:13-15,
    #          price_ref:16-18, size_ref:19-20, time_ref:21-25]
    TOK_LENS = np.array((1, 1, 3, 2, 1, 3, 2, 3, 3, 2, 2, 3))  # sum=26
    ...
    # 26tok: [evt:0, dir:1, price:2-4, size:5-6, dt_s:7, dt_ns:8-10, time_s:11-12, time_ns:13-15, ...]
    TIME_START_I = 11
    TIME_END_I = 15
    ...
    def preproc(self, m, b, allowed_event_types=[1,2,3,4]):
        ...
        # SIZE: 2 base-100 digits, range 0-9999 (same as 24tok)
        ...
        # 26tok: +/-999,999 price levels (after /100 tick scaling) = +/-$9,999.99
        m.price = self._preproc_prices(m.price, p_ref, p_lower_trunc=-99999900, p_upper_trunc=99999900)
        m = m.iloc[1:]
